<template>
	<div class="analysis-results-list">
		...
		<div class="results-toolbar">
			...
			<button v-if="gisStore.analysisResults.length > 0" @click="handleClearAll" class="clear-btn" title="清空所有结果">
		...
		<div class="results-container">
			<div v-for="result in gisStore.analysisResults" :key="result.id"
				:class="['result-item', { active: result.id === gisStore.selectedResultId }]" @click="handleSelect(result.id)">
				<!-- 结果头部 -->
...
function handleLocate(result: AnalysisResult): void {
	...
	// 将位置转为经纬度，然后设置适当的观察高度
	const cartographic = Cesium.Cartographic.fromCartesian(result.position)
	const destination = Cesium.Cartesian3.fromRadians(
		cartographic.longitude,
		cartographic.latitude,
		cartographic.height + 2000 // 抬高 2000m
	)
...
// 查看图表（仅剖面）
function handleViewChart(result: AnalysisResult): void {
	if (result.type !== 'profile') return
	// 通过 store 的 overlay 状态驱动 ProfileChart 显示
	gisStore.setAnalysisResult('profile', result.data)
}
