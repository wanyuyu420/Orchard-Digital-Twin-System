<template>
	<GlassPanel title="仿真配置" icon="fa-solid fa-sliders" :actions="['import', 'save']">
		...
		<div v-if="(state.engine === 'flood' || state.engine === 'hydro') && store.floodEvents.length > 0"
			class="form-group">
		...
		<!-- Dynamic Forms for flood/hydro -->
		<div v-if="state.engine === 'flood' || state.engine === 'hydro'" class="form-group">
			<label class="form-label">入库流量 (m³/s)</label>
			<div class="range-wrapper">
				<input type="range" min="0" max="5000" step="10" v-model.number="state.flow" class="form-range" />
		...
		<!-- Dam Safety Forms -->
		<div v-else-if="state.engine === 'dam'" class="form-group">
