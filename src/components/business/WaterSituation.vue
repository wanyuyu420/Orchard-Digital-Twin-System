	...
	<GlassPanel>
		<template #default>
			...
			<div class="chart-wrapper">
				<WaterLevelChart :data="chartData" height="120px"
					:loading="!realtimeStore.isConnected && chartData.length === 0" title="" />
			</div>
			...
			<div class="chart-wrapper">
				<FlowRateChart :data="flowRateChartData" height="100px"
					:loading="!realtimeStore.isConnected && flowRateChartData.length === 0" title="" />
			</div>
			<div class="flow-stats">
				<div class="stat-item" v-for="station in flowRateStations.slice(0, 3)" :key="station.sensor_id">
					...
					<span class="flow-value">{{ station.history[station.history.length - 1]?.value?.toFixed(3) ?? '-' }}
						m³/s</span>
				</div>
			</div>
		</template>
