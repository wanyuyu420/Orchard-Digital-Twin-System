 ...
 */
export const getFloodScenarioDetail = async (
  scenarioId: number
): Promise<FloodScenarioDetail> => {
  const response = await apiClient.get<FloodScenarioDetail>(
    `/v1/flood/scenarios/${scenarioId}`
  );
  return response.data;
};
 ...
 */
export const getFloodFrame = async (
  scenarioId: number,
  progress: number
): Promise<FloodFrame> => {
  const response = await apiClient.get<FloodFrame>(
    `/v1/flood/scenarios/${scenarioId}/frame`,
    { params: { progress } }
  );
  return response.data;
};
