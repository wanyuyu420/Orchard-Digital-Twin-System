 ...
 */
export async function setupOrchardPreviewBasemap(viewer: any): Promise<void> {
  addGaodeImagery(viewer)
  addDomImagery(viewer)
  await setupDemTerrain(viewer)
}
...
export async function setupDemTerrain(viewer: any): Promise<void> {
  // 1) 加载 dem.js(内容为 window.DEM = {...},用 Function 构造器在全局执行)
  try {
  ...
  // 2) 双线性插值高程(与 viewer.html demHeight 一致)
  ...
  // 3) 自定义 TerrainProvider(移植自 viewer.html)
